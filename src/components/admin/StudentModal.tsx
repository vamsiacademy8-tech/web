...
export const StudentModal: React.FC<StudentModalProps> = ({
  ...
  onSave,
  initialData,
  defaultStudentIdCode = '100',
}) => {
  const [name, setName] = useState('');
  const [email, setEmail] = useState('');
  const [password, setPassword] = useState('');
  const [phone, setPhone] = useState('');
  const [studentIdCode, setStudentIdCode] = useState('100');
          ...
          <div className="grid grid-cols-2 gap-3">
            <div>
              ...
              <input
                ...
                readOnly
                value={studentIdCode}
                className="w-full px-3.5 py-2.5 rounded-xl border border-slate-200 bg-slate-100 text-slate-700 outline-none text-sm font-mono font-bold cursor-not-allowed"
              />
            </div>
